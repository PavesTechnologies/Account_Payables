...
class VendorAddress(Base):
    ...
    vendor_tax: Mapped[list["VendorTax"]] = relationship('VendorTax',back_populates='vendor_address')
...
class VendorTax(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['vendor_address_id'], ['ap.vendor_address.vendor_address_id'], ondelete='CASCADE', name='vendor_tax_vendor_address_id_fkey'),
        PrimaryKeyConstraint('vendor_tax_id', name='vendor_tax_pkey'),
        UniqueConstraint('vendor_address_id', 'registration_type', name='vendor_tax_vendor_address_id_registration_type_key'),
        Index('idx_vendor_tax_address', 'vendor_address_id'),
        {'schema': 'ap'}
    )

    vendor_tax_id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    vendor_address_id: Mapped[int] = mapped_column(Integer, nullable=False)
